...
| **[humain]** Le SDK Sentry navigateur coûte 48 ko gz pour surveiller zéro script | 6/8 | **Constaté** en construisant avec un DSN factice : `PUBLIC_SENTRY_DSN` présent au build embarque 145 ko (48 ko gz) de JavaScript sur chaque page, alors que le site n'envoie aujourd'hui **aucun** script au client — il n'y a littéralement rien à surveiller. Le §2 de `CLAUDE.md` a été précisé en conséquence. **À vérifier en console : la variable ne doit pas être définie sur Netlify** tant que le site n'a pas d'îlot. Le budget JS de `scripts/check-deploy.mjs` (20 ko servis) échoue bruyamment si elle l'est |
