...
| Aucun check ne regarde ce que la preview sert | 5/8 | Le déploiement est réparé, mais rien n'empêche la panne de revenir : quatre PR ont été mergées le 5/8 avec `verify` vert et « Deploy Preview ready! » alors que le site servait 404. **Il manque un check exigeant un 200 et le `<h1>` attendu sur la preview.** C'est la condition pour que le §1 tienne — à écrire avec J1-12, qui en a besoin de toute façon |
