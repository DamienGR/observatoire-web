...
| **Le site déployé répond 404 sur `/`** | 5/8 | Constaté en vérifiant la deploy preview de la PR #20. Netlify annonce « Deploy Preview ready! » et sert pourtant sa **propre page 404 statique** depuis le cache edge (`cache-status: "Netlify Edge"; hit`, `etag` Netlify) : la fonction SSR n'est jamais invoquée. Vaut pour la production (`observatoireweb.netlify.app`) **et** pour des previews antérieures (#10, #18, #19) — donc pré-existant, sans rapport avec les PR de dépendances. C'est le cas exact que le §1 de `CLAUDE.md` désigne : « si la CI est verte et que le produit est cassé, c'est la CI qu'il faut corriger ». Aucun check ne regarde ce que la preview sert réellement. **À traiter en priorité, avant J1-12** : à ajouter à J1-09 ou en tâche propre, avec un check qui exige un 200 et un `<h1>` attendu |
