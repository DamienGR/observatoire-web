...
| **Netlify ne construit pas le projet : il publie la racine du dépôt** | 5/8 | Constaté en vérifiant la deploy preview de la PR #20. `/` répond 404, mais `/package.json`, `/CLAUDE.md`, `/docs/brief.md` et `/src/pages/index.astro` répondent **200 avec leur contenu réel**, tandis que `/README.md` répond 404 — fichier qui n'existe pas dans le dépôt. La correspondance est exacte : Netlify sert l'arborescence Git en statique. Le build n'est jamais exécuté, la fonction SSR jamais déployée, et `/` échoue faute d'`index.html` à la racine. Il n'y a **aucun `netlify.toml`** dans le dépôt, et les réglages du site n'ont manifestement ni commande de build ni répertoire de publication. Conséquences : aucun en-tête du §7 n'a jamais été en vigueur (seul `Strict-Transport-Security`, ajouté par Netlify), et J1-12 est bloquée sur une preview qui ne sert rien. Vaut aussi pour la production et pour les previews #10, #18, #19 — pré-existant, sans rapport avec les PR de dépendances. Le §1 le décrit mot pour mot : « si la CI est verte et que le produit est cassé, c'est la CI qu'il faut corriger ». **Priorité haute** : `netlify.toml` versionné, plus un check qui exige un 200 et le `<h1>` attendu |
