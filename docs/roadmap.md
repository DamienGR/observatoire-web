...
| Plafond Dependabot atteint sur npm | 5/8 | `open-pull-requests-limit: 5` était saturé par les majeures : le groupe `dev-dependencies` n'a produit aucune PR, sans qu'on puisse distinguer « à jour » de « masqué ». Vérifier à la récolte suivante que le groupe reparaît une fois les majeures soldées |
