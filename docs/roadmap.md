...
| Domaine mort en dur dans le code | 6/8 | `astro.config.mjs` retombe sur `https://observatoire-web.fr` et `.env.example` porte la même valeur, alors que ce domaine ne résout pas. Sans effet visible aujourd'hui — la page est en `noindex` — mais les URL canoniques du site pointent vers le vide dès que l'indexation sera ouverte. À traiter avec J1-09 |
