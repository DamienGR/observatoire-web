...
| Le check `deploy` ne couvre pas la production | 6/8 | Restreint aux `pull_request` : Netlify publie un statut de commit pour les deploy previews mais **pas** pour les déploiements de production — mesuré sur `7e36ec2`, où le job a sondé cinq minutes en voyant `0 netlify status(es) seen`. Vérifier la production demande son hôte, que le §6 garde hors du diff : il faut `SITE_URL` en **variable Actions**, réglage de console donc **[humain]**. Peu de perte en attendant, `main` étant protégée : tout commit y arrive par une PR que ce job a déjà gardée |
