...
| Trois advisories résiduelles après la récolte du 5/8 | 5/8 | La montée de version fait passer `pnpm audit` de 4 hautes / 7 modérées / 4 basses à **1 haute / 2 modérées** : `sharp` (0.34.5, CVE libvips, transitive via Astro et `@netlify/images`), `esbuild` et `qs`. Aucune n'est corrigeable par une plage directe — il faudrait un `overrides` pnpm, c'est-à-dire imposer à Astro une version qu'il n'a pas testée. **Hors du périmètre de cette PR** (§12 : on ouvre un ticket, on n'absorbe pas) : à traiter séparément, en priorité `sharp`. Aucune n'est atteignable depuis la surface publique aujourd'hui — le site ne sert pas d'image et `esbuild` n'est exposé qu'en développement |
| Alias `~` non miroité dans Vitest | 5/8 | `tsconfig.json` déclare `paths: { "~/*": ["./src/*"] }` et `astro.config.mjs` déclare l'alias Vite correspondant, mais `vitest.config.ts` n'en déclare aucun. Aucun fichier n'utilise `~/` aujourd'hui, donc la brèche est latente — mais J1-06, J1-07 et J1-08 écrivent toutes dans `src/lib/` et l'emploieront naturellement. Le typecheck passera, le test échouera à la résolution. À miroiter **avant** le premier import `~/`, pas après |
