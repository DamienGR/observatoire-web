...
describe('checkUrl — scheme', () => {
  ...
  it('rejects http unless the fallback is explicitly asked for', () => {
    const rejected = checkUrl('http://www.ville-exemple.fr/');
  ...
  it.each([
    ...
    expect(checkUrl(raw, { allowHttp: true }).reason).toBe('forbidden-scheme');
  });
});
...
describe('checkUrl — credentials in the URL', () => {
  ...
  it.each([
    ...
    expect(checkUrl(raw).reason).toBe('embedded-credentials');
  });
...
describe('checkUrl — IP literals are judged without asking DNS', () => {
  ...
  it('normalises an IPv4 address written as a single integer', () => {
    ...
    const rejected = checkUrl('https://2130706433/');
  ...
  it('normalises an IPv4 address written in hex', () => {
    expect(checkUrl('https://0x7f000001/').address?.effectiveAddress).toBe('127.0.0.1');
  });
...
describe('checkUrl — reserved hostnames', () => {
  ...
  it.each([
    ...
    expect(checkUrl(raw).reason).toBe('reserved-hostname');
  });
...
describe('checkUrl — output', () => {
  it('returns a parsed URL the caller can use directly', () => {
    const checked = checkUrl('https://www.ville-exemple.fr/accessibilite?a=1#b');
